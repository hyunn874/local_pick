...
| `LocalPassScreen` | `LocalPass` tab | 로그인 필요. 게스트면 로그인 유도 empty state | `Settings`, `PassHistory`, `ResidentVerification`, `Map` | 보유 로컬패스, 화면 진입 시 잔액 동기화, 사용 내역 API, 로컬패스 사용 API, 획득 방법, 내 게시글 채택 진행률, 로그아웃 |
...
- API 지역 좌표가 없으면 `src/data/regionCoordinates.js`의 `regionCode`별 좌표를 사용하고, 이후 시·도/기본 좌표를 사용한다.
- 채택 명소 API에 실제 좌표가 있으면 첫 명소 좌표로 이동하고 명소 확대 수준을 사용한다.
- `NaverMapView`는 `forwardRef`로 지도 ref를 전달하며 `animateCameraTo`로 지역 선택 후 중심을 이동한다. 초기 위치만 `initialCamera`로 설정한다.
...
- `ChatRoomScreen`: `GET /api/posts?region=...`, `POST /api/posts`, `POST /api/posts/{postId}/like`, `POST /api/posts/{postId}/adopt`를 사용한다. 게시글 작성 성공 후 목록을 재조회한다.
...
- `LocalPassScreen`: `GET /api/local-pass/balance`, `GET /api/local-pass/history`, `POST /api/local-pass/use`를 사용한다.
- `PassHistoryScreen`: `GET /api/local-pass/history`를 사용한다.
- `MapScreen`: 선택 지역의 좌표가 없으면 `GET /api/regions/{regionCode}`를 조회한 뒤 프론트 좌표 테이블로 폴백한다.
